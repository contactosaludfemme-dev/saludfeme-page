  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto grid max-w-6xl items-center gap-10 px-5 lg:grid-cols-[1.06fr_0.94fr] lg:gap-14">
        {/* Texto */}
        <div>
          <span className="mb-6 inline-flex items-center gap-2 rounded-full border border-vino-700/20 bg-white/85 px-4 py-1.5 text-[0.8rem] font-semibold text-vino-700 shadow-suave backdrop-blur">
          ...
          <h1>
            <span className="block font-titulo text-[clamp(2.1rem,5.2vw,3.7rem)] font-bold uppercase leading-[0.95] tracking-[-0.02em] text-vino-700">
              Tu salud, tu espacio,
            </span>
            <span className="relative mt-1 inline-block font-mano text-[clamp(2.5rem,5.8vw,4.2rem)] font-bold leading-[1] text-rosa-650">
              tus decisiones
              <Destello className="absolute -right-5 -top-2 size-5 text-rosa-500" />
            </span>
          </h1>
          ...
          <p className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-1 text-[0.9rem] font-semibold text-vino-700">
            <span>📍 Talca</span>
            <span>📍 Linares</span>
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            <button
              type="button"
              onClick={() => abrir()}
              className="inline-flex items-center gap-2 rounded-full bg-vino-700 px-8 py-4 font-titulo font-semibold text-white shadow-media transition-all hover:-translate-y-0.5 hover:bg-vino-800 hover:shadow-fuerte"
            >
              Agendar mi hora
            ...
            <a
              ...
              className="inline-flex items-center gap-2 rounded-full border-2 border-vino-700/40 bg-white/60 px-7 py-4 font-titulo font-semibold text-vino-700 backdrop-blur transition-colors hover:border-vino-700 hover:bg-white"
            >
          ...
          <dl className="mt-9 flex flex-wrap items-center gap-x-7 gap-y-3 border-t border-vino-700/15 pt-6">
            {TRAYECTORIA_CIFRAS.map((s, i) => (
              <div key={s.lbl} className="flex items-center gap-7">
                <div>
                  <dt className="sr-only">{s.lbl}</dt>
                  <dd>
                    <span className="block font-titulo text-[1.7rem] font-bold leading-none text-vino-700">
                      {s.num}
                    </span>
                ...
                {i < TRAYECTORIA_CIFRAS.length - 1 && (
                  <span aria-hidden className="hidden h-8 w-px bg-vino-700/15 sm:block" />
                )}
              </div>
            ))}
          </dl>
        ...
        <div className="relative mx-auto w-full max-w-sm">
          ...
          <div className="relative aspect-[4/5] overflow-hidden rounded-[2.5rem] border-4 border-white/70 bg-white/55 shadow-fuerte backdrop-blur-sm">
            <div className="grid size-full place-content-center place-items-center gap-3 p-8 text-center">
              ...
              <p className="font-titulo text-[0.9rem] font-semibold text-vino-700">
          ...
          <div className="absolute -left-3 bottom-8 flex items-center gap-3 rounded-2xl bg-white p-3.5 shadow-fuerte">
            ...
            <span className="text-[0.8rem] leading-tight">
              <strong className="block font-titulo text-vino-700">
                Matrona titulada
              </strong>
